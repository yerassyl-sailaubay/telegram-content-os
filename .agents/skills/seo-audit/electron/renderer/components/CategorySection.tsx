...
export function CategorySection({
  ...
  const filteredRules =
    filter === "all" ? category.results : category.results.filter((r) => r.status === filter);
  ...
  return (
    <section id={`category-${category.categoryId}`} className="mb-4">
      <button
        ...
        className="flex w-full items-center justify-between rounded-lg p-3 transition-colors hover:bg-[var(--color-bg-hover)]"
      >
