...
export function RuleCard({ rule, metadata }: RuleCardProps) {
  ...
  const suggestion = (details.suggestion ?? details.recommendation ?? details.note) as
    | string
    | undefined;
  ...
  return (
    <div
      className="rounded-lg border border-[var(--color-border-subtle)] p-4 transition-colors hover:bg-[var(--color-bg-hover)]"
      id={`rule-${rule.ruleId}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {showDescription && (
            <p className="mt-0.5 text-xs italic" style={{ color: "var(--color-text-muted)" }}>
              {ruleDescription}
            </p>
          )}
          ...
          {pageUrl && (
            <div className="mt-1.5">
              <a
                ...
                className="inline-block rounded px-2 py-0.5 font-mono text-xs transition-opacity hover:opacity-80"
                style={{
          ...
          {fixText && (
            <div className="mt-3 py-2 pl-3" style={{ borderLeft: "3px solid var(--color-accent)" }}>
          ...
          {hasExtraDetails && (
            <div className="mt-2">
              ...
              >
                {showDetails
                  ? "Hide details"
                  : `View details (${listFields.reduce((n, f) => n + f.items.length, 0) + statFields.length} items)`}
              </button>
                ...
                <div className="mt-2 space-y-2">
                  ...
                  {listFields.map(({ key, items }) => (
                    <div key={key}>
                      ...
                      <div
                        className="max-h-48 space-y-0.5 overflow-y-auto rounded-md p-2"
