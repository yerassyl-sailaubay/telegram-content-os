...
export async function runDbStats(options: DbStatsOptions): Promise<void> {
  ...
  if (fs.existsSync(auditsDbPath)) {
    ...
    if (options.verbose) {
      const { getAuditsDatabase, closeAuditsDatabase } =
        await import("../storage/audits-db/index.js");
...
export async function runDbRestore(_options: DbRestoreOptions): Promise<void> {
  ...
  if (restored) {
    ...
    console.log(
      chalk.dim("Note: SQLite databases were not deleted. You may remove them manually if needed."),
    );
  } else {
