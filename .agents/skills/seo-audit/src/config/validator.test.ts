...
describe("formatValidationResult", () => {
  ...
  it("should format errors", () => {
    const result = {
      valid: false,
      errors: [{ path: "crawler.max_pages", message: "must be between 1 and 10000", value: 0 }],
      warnings: [],
    };
  ...
  it("should format warnings", () => {
    const result = {
      valid: true,
      errors: [],
      warnings: [{ path: "crawler.max_pages", message: "is high", value: 7500 }],
    };
  ...
  it("should format both errors and warnings", () => {
    const result = {
      valid: false,
      errors: [{ path: "output.format", message: "invalid format" }],
      warnings: [{ path: "crawler.concurrency", message: "is high" }],
    };
