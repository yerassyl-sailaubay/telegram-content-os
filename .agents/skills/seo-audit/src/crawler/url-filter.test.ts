...
describe("UrlFilter", () => {
  ...
  describe("normalizeUrl", () => {
    it("should remove hash fragments", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/page#section")).toBe(
    ...
    it("should remove default tracking params", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/page?utm_source=google&utm_medium=cpc")).toBe(
        "https://example.com/page",
      );

      expect(filter.normalizeUrl("https://example.com/page?gclid=abc123")).toBe(
        "https://example.com/page",
      );

      expect(filter.normalizeUrl("https://example.com/page?fbclid=xyz")).toBe(
        "https://example.com/page",
      );
    });
    ...
    it("should keep non-tracking params", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/search?q=hello")).toBe(
        "https://example.com/search?q=hello",
      );

      expect(filter.normalizeUrl("https://example.com/page?id=123&utm_source=test")).toBe(
    ...
    it("should only keep allowed params when allowQueryParams is set", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/products?page=2&sort=name&filter=new")).toBe(
    ...
    it("should sort query params for consistent normalization", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/page?c=3&a=1&b=2")).toBe(
    ...
    it("should remove trailing slash except for root", () => {
      ...
      expect(filter.normalizeUrl("https://example.com/")).toBe("https://example.com/");
    });
    ...
    it("should handle custom drop prefixes", () => {
      ...
      expect(
        filter.normalizeUrl("https://example.com/page?track_source=email&ref_id=123&id=456"),
