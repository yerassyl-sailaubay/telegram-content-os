...
export const focusVisibleRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    if (hasFocusVisibleSupport && issues.length <= 1) {
      return warn("a11y-focus-visible", "Focus outline removed but :focus-visible detected", {
        issues,
        hasFocusVisibleSupport: true,
    ...
    if (issues.length > 3) {
      return fail("a11y-focus-visible", `Found ${issues.length} focus indicator issues`, {
        issues: issues.slice(0, 10),
        totalIssues: issues.length,
      });
    }

    return warn("a11y-focus-visible", `Found ${issues.length} potential focus indicator issue(s)`, {
      issues,
