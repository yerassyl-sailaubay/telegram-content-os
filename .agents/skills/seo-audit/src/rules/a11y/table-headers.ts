...
export const tableHeadersRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    $("table").each((index, el) => {
      ...
      const isLayoutTable = role === "presentation" || role === "none" || $table.hasClass("layout");
    ...
    if (noHeaderIssues.length > 0) {
      return fail("a11y-table-headers", `${noHeaderIssues.length} table(s) missing header cells`, {
        issues: issues.slice(0, 10),
        dataTables: dataTables.length,
      });
    }

    return warn("a11y-table-headers", `Found ${issues.length} table accessibility issue(s)`, {
      issues,
      dataTables: dataTables.length,
      tablesWithHeaders,
    });
