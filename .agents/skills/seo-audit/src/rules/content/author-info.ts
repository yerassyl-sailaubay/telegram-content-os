...
/**
 * Rule: Check for author information (E-E-A-T signals)
 *
 * Checks for:
 ...
 */
export const authorInfoRule = defineRule({
  id: "content-author-info",
  name: "Author Info",
  description: "Checks for author markup and attribution for E-E-A-T signals",
  category: "content",
  weight: 3,
  run: async (context: AuditContext) => {
    ...
    for (const selector of bylineSelectors) {
      ...
      if (element.length > 0) {
        ...
        if (text && text.length < 100) {
          ...
          signals.push(
            `Byline element (${selector}): "${text.substring(0, 50)}${text.length > 50 ? "..." : ""}"`,
          );
    ...
    if (signals.length > 0) {
      return pass(
        "content-author-info",
        `Author information found (${signals.length} signal${signals.length > 1 ? "s" : ""})`,
        {
          signals,
          impact:
            "Author attribution supports E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness)",
        },
      );
    }

    return warn("content-author-info", "No author information found", {
      signals: [],
      recommendation:
