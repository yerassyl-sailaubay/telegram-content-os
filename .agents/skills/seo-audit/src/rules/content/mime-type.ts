  ...
  run: async (context: AuditContext) => {
    ...
    if (!mimeType.includes("text/html") && !mimeType.includes("application/xhtml+xml")) {
      return fail("content-mime-type", `Incorrect MIME type: ${mimeType}`, {
        ...
        recommendation:
          "HTML pages should be served with text/html or application/xhtml+xml MIME type",
    ...
    if (!isUtf8) {
      return warn("content-mime-type", `Non-UTF-8 charset detected: ${charset}`, {
        header: contentType,
        mimeType,
        charset,
    ...
    return pass("content-mime-type", "Content-Type header is correctly configured", {
      header: contentType,
      mimeType,
      charset,
    });
