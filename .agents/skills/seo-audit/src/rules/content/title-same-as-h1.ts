...
export const titleSameAsH1Rule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!title || !h1) {
      return pass("content-title-same-as-h1", "Title or H1 is missing (handled by other rules)", {
        title: title || null,
        h1: h1 || null,
        reason: "skipped",
      });
    ...
    if (normalizedTitle === normalizedH1) {
      return warn("content-title-same-as-h1", "Title tag and H1 heading are identical", {
        title,
        h1,
        normalizedTitle,
        normalizedH1,
        impact: "Identical title and H1 miss an opportunity for keyword variety in SERPs",
        recommendation:
    ...
    return pass("content-title-same-as-h1", "Title tag and H1 heading are different", {
      title,
      h1,
      normalizedTitle,
      normalizedH1,
