...
export const canonicalConflictingRule = defineRule({
  ...
  description:
    "Checks if multiple canonical URLs are specified via HTML and HTTP header pointing to different URLs",
  category: "core",
  weight: 8,
  run: async (context: AuditContext) => {
    ...
    return fail(
      ...
      `Conflicting canonical URLs: HTML tag specifies "${htmlCanonical}" but Link header specifies "${headerCanonical}"`,
      {
        ...
        match: false,
        recommendation:
          'Remove one canonical source to avoid conflicting signals; prefer the HTML <link rel="canonical"> tag',
      },
    );
  },
});
