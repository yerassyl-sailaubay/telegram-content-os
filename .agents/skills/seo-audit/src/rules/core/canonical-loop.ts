...
export const canonicalLoopRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    // No canonical tag
    if (!canonicalHref) {
      return pass("core-canonical-loop", "No canonical tag present; no loop risk", {
    ...
    try {
      ...
      return pass("core-canonical-loop", "Canonical URL could not be parsed; skipping loop check", {
        canonicalHref,
      });
    }
    ...
    if (normalizedPage === normalizedCanonical) {
      return pass("core-canonical-loop", "Canonical is self-referencing (best practice)", {
        pageUrl: url,
    ...
    return warn(
      ...
      `Canonical points to a different URL ("${canonicalUrl.href}"); verify the target does not canonicalize back to this page`,
      {
        ...
        recommendation:
          "Ensure the canonical target page does not point back to this page, which would create a loop that confuses search engines",
      },
    );
  },
});
