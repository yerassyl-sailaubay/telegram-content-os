...
export const canonicalPresentRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (canonicalElement.length === 0) {
      return fail("core-canonical-present", 'No <link rel="canonical"> tag found in the document', {
        found: false,
      });
    }
    ...
    if (!href) {
      return fail("core-canonical-present", "Canonical link tag exists but has no href", {
        found: true,
        empty: true,
      });
    }

    return pass("core-canonical-present", "Canonical URL tag is present", {
      found: true,
      canonicalUrl: href,
    });
  },
