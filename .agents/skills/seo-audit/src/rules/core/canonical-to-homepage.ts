...
export const canonicalToHomepageRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    try {
      ...
      return pass("core-canonical-to-homepage", "Could not parse page URL; skipping check", {
        url,
      });
    }
    ...
    if (canonicalPath === "/" || canonicalPath === "") {
      return fail(
        ...
        `Non-homepage page canonicalizes to homepage: canonical is "${canonicalHref}" but page path is "${pageUrl.pathname}"`,
        {
          ...
          recommendation:
            "Set the canonical URL to the current page URL (self-referencing canonical) unless this is an intentional redirect",
        },
      );
    }

    return pass("core-canonical-to-homepage", "Canonical URL does not point to the homepage", {
      canonicalUrl: canonicalUrl.href,
      canonicalPath,
      pagePath: pageUrl.pathname,
    });
