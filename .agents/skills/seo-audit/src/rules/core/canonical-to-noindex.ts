...
export const canonicalToNoindexRule = defineRule({
  ...
  description:
    "Checks if a page has both noindex and a canonical pointing to a different URL (conflicting signals)",
  category: "core",
  weight: 7,
  run: async (context: AuditContext) => {
    ...
    return warn(
      ...
      `Page has both noindex and a canonical pointing to "${canonicalUrl.href}" (conflicting signals)`,
      {
        ...
        recommendation:
          "Remove the noindex directive to let the canonical signal work, or remove the canonical and keep noindex if the page should not be indexed",
      },
    );
  },
});
