...
export const canonicalValidRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (canonicalElement.length === 0) {
      return warn("core-canonical-valid", 'No <link rel="canonical"> tag found in the document', {
        found: false,
      });
    }
    ...
    if (!href) {
      return fail("core-canonical-valid", "Canonical link tag exists but has no href", {
        found: true,
        empty: true,
      });
    }
    ...
    if (!isValidAbsoluteUrl(href)) {
      return fail("core-canonical-valid", `Canonical URL is not a valid absolute URL: "${href}"`, {
        canonicalUrl: href,
        isAbsolute: false,
      });
    }

    return pass("core-canonical-valid", "Canonical URL is a valid absolute URL", {
      canonicalUrl: href,
      isAbsolute: true,
    });
  },
