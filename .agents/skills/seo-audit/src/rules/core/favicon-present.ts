...
export const faviconPresentRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (iconElement.length === 0 && shortcutIconElement.length === 0) {
      return fail("core-favicon-present", "No favicon link tag found in the document", {
        found: false,
      });
    }
    ...
    if (!href) {
      return fail("core-favicon-present", "Favicon link tag exists but has no href", {
        found: true,
        empty: true,
      });
    }

    return pass("core-favicon-present", "Favicon link tag is present", {
      found: true,
      faviconUrl: href,
      rel,
    });
