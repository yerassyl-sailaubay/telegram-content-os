...
export const h1PresentRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (h1Elements.length === 0) {
      return fail("core-h1-present", "No <h1> tag found in the document", {
        found: false,
        count: 0,
      });
    }
    ...
    if (!h1Text) {
      return fail("core-h1-present", "H1 tag exists but is empty", {
        found: true,
        empty: true,
    ...
    return pass("core-h1-present", "H1 tag is present", {
      found: true,
      count: h1Elements.length,
      text: h1Text,
    });
