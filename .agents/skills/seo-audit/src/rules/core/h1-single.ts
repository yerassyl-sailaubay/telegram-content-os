...
export const h1SingleRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (count === 0) {
      return fail("core-h1-single", "No <h1> tag found in the document", { count: 0 });
    }
    ...
    return pass("core-h1-single", "Document has exactly one <h1> tag", {
      count: 1,
      text: h1Elements.first().text().trim(),
    });
  },
});
