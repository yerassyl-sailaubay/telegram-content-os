...
export const titleLengthRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (titleElement.length === 0) {
      return fail("core-title-length", "No <title> tag found in the document", { found: false });
    }

    const titleText = titleElement.first().text().trim();

    if (!titleText) {
      return fail("core-title-length", "Title tag exists but is empty", { found: true, length: 0 });
    }
    ...
    return pass("core-title-length", `Title length is optimal (${length} characters)`, {
      length,
