...
export const titlePresentRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (titleElement.length === 0) {
      return fail("core-title-present", "No <title> tag found in the document", { found: false });
    }

    const titleText = titleElement.first().text().trim();

    if (!titleText) {
      return fail("core-title-present", "Title tag exists but is empty", {
        found: true,
        empty: true,
      });
    }

    return pass("core-title-present", "Title tag is present", { found: true, title: titleText });
  },
});
