...
export const viewportPresentRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (viewportElement.length === 0) {
      return fail("core-viewport-present", 'No <meta name="viewport"> tag found in the document', {
        found: false,
      });
    }
    ...
    if (!content) {
      return fail("core-viewport-present", "Viewport meta tag exists but has no content", {
        found: true,
        empty: true,
      });
    }

    return pass("core-viewport-present", "Viewport meta tag is present", {
      found: true,
      viewport: content,
    });
  },
