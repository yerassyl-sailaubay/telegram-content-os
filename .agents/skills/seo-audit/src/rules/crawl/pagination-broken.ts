...
export const paginationBrokenRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!nextHref && !prevHref) {
      return pass("crawl-pagination-broken", 'No pagination links found (rel="next"/"prev")', {
        hasPagination: false,
      });
    }
    ...
    if (issues.length > 0) {
      ...
      return fail("crawl-pagination-broken", `Pagination ${brokenRels} link(s) have invalid URLs`, {
        ...details,
