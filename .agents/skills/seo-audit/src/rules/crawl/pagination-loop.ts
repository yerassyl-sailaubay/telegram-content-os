...
export const paginationLoopRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!nextHref && !prevHref) {
      return pass("crawl-pagination-loop", 'No pagination links found (rel="next"/"prev")', {
        hasPagination: false,
      });
    }
    ...
    if (loops.length > 0) {
      ...
      return fail(
        ...
        `Pagination ${loopRels} points back to the current page (infinite loop)`,
        {
          ...details,
          impact:
            "Pagination loops waste crawl budget and prevent proper indexing of paginated series",
          recommendation:
            "Fix pagination links to point to the correct next/previous page in the series",
        },
      );
    }

    return pass("crawl-pagination-loop", "Pagination links do not create a loop", details);
  },
});
