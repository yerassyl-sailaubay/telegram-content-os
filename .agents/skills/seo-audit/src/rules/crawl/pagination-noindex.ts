    ...
    if (isNoindexed) {
      return warn(
        ...
        `Paginated page has noindex directive (via ${sources.join(", ")})`,
        {
          ...details,
          impact:
            "Noindexing paginated pages prevents content discovery and can break pagination series",
          recommendation:
            'Remove noindex from paginated pages; use rel="canonical" to consolidate signals instead',
        },
      );
    }

    return pass("crawl-pagination-noindex", "Paginated page is not noindexed (correct)", details);
  },
});
