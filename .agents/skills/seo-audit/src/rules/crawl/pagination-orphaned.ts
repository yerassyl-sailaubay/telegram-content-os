...
export const paginationOrphanedRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!matches) {
      return pass("crawl-pagination-orphaned", "Page does not appear to be paginated", {
        isPaginated: false,
    ...
    return warn(
      ...
      `URL appears paginated (${label}) but is missing rel="next"/"prev" link tags`,
      {
        ...
        recommendation:
          'Add <link rel="next"> and/or <link rel="prev"> to declare the pagination relationship',
      },
    );
  },
});
