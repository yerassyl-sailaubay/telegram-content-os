...
export const contentStructureRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    const hasMainArea =
      $("main").length > 0 || $("article").length > 0 || $('[role="main"]').length > 0;
    ...
    if (hasHierarchy) {
      ...
      missingSignals.push("hierarchical headings (h1 followed by h2, h3, etc.)");
    }
    ...
    if (hasParagraphs) {
      ...
      missingSignals.push("paragraph content (at least 30 words of prose in <p> elements)");
    }
