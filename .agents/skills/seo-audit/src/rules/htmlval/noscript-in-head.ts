...
const PERMITTED_NOSCRIPT_HEAD_ELEMENTS = new Set(["link", "style", "meta"]);

export const noscriptInHeadRule = defineRule({
  id: "htmlval-noscript-in-head",
  name: "Noscript in Head",
  description:
    "Checks that <noscript> inside <head> contains only permitted elements (link, style, meta)",
  category: "htmlval",
  weight: 5,
  run: async (context: AuditContext) => {
    ...
    if (headNoscripts.length === 0) {
      return pass("htmlval-noscript-in-head", "No <noscript> elements found in <head>");
    }
