...
export const modernFormatRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    if (images.length === 0) {
      return pass("images-modern-format", "No images found on page", { imageCount: 0 });
    }

    // Check for picture elements with source elements (which often provide modern formats)
    const pictureElements = $("picture").length;
    const sourcesWithModernFormats = $(
      'picture source[type*="webp"], picture source[type*="avif"]',
    ).length;
    ...
    const modernPercentage =
      totalWithFormat > 0 ? ((modernImages.length / totalWithFormat) * 100).toFixed(1) : "0";
    ...
    if (modernImages.length === 0 && legacyImages.length > 0 && sourcesWithModernFormats === 0) {
      return warn("images-modern-format", `No modern image formats detected (0% WebP/AVIF usage)`, {
