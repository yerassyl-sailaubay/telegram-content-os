...
export const canonicalMismatchRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    const comparison = compareAttribute(context.$, rendered$, 'link[rel="canonical"]', "href");

    if (!comparison) {
      return pass("js-canonical-mismatch", "Unable to compare canonical tags");
    }
    ...
    if (comparison.differs) {
      return fail("js-canonical-mismatch", "Canonical URL changed after JavaScript execution", {
        ...
        recommendation:
          "Ensure the canonical URL is consistent in both raw HTML and after JS rendering",
      });
    }
