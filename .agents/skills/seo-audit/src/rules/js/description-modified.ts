...
export const descriptionModifiedRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!comparison) {
      return pass("js-description-modified", "Unable to compare meta description elements");
    }
    ...
    if (comparison.differs) {
      return warn(
        ...
        "Meta description was modified by JavaScript execution",
        {
          ...
          recommendation:
            "Set the correct meta description in server-side HTML rather than modifying it with JavaScript",
        },
      );
    }
