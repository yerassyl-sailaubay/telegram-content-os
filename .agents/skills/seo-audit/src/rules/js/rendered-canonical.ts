...
export const renderedCanonicalRule = defineRule({
  ...
  description:
    "Checks that a canonical link tag exists in the rendered DOM after JavaScript execution",
  category: "js",
  weight: 8,
    ...
    if (!canonicalHref) {
      return fail(
        ...
        "Canonical link tag is missing in the rendered DOM after JavaScript execution",
        {
          ...
          recommendation:
            'Ensure your JavaScript framework injects <link rel="canonical"> during rendering',
        },
      );
    }

    return pass("js-rendered-canonical", "Canonical link tag is present in the rendered DOM", {
