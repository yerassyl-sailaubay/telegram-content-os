    ...
    if (rawLen < 100 && renderedLen > 500) {
      return fail(
        ...
        "Page is essentially empty without JavaScript; all content is client-side rendered",
        {
          ...
          recommendation:
            "Implement server-side rendering (SSR) or static generation to include content in raw HTML",
        },
      );
    }
    ...
    if (renderedLen > rawLen * 3) {
      return warn(
        ...
        "Most page content is injected by JavaScript; raw HTML has significantly less content",
        {
          ...details,
          impact:
            "Search engines may not index all content if JavaScript rendering is delayed or fails",
          recommendation:
            "Consider server-side rendering for critical content to ensure indexability",
        },
      );
    }

    return pass("js-rendered-content", "Page content is mostly present in raw HTML", details);
  },
});
