    ...
    if (renderedLinks > rawLinks * 2 && linkDifference > 10) {
      return warn(
        ...
        `JavaScript adds ${linkDifference} links to the page; most links are not in raw HTML`,
        {
          ...
          recommendation:
            "Include critical navigation and internal links in server-side HTML for reliable discovery",
        },
      );
    }

    return pass("js-rendered-links", "Links are mostly present in raw HTML", details);
  },
});
