...
export const titleModifiedRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!comparison) {
      return pass("js-title-modified", "Unable to compare title elements");
    }
    ...
    if (comparison.differs) {
      return warn("js-title-modified", "Title was modified by JavaScript execution", {
        rawTitle: comparison.rawText || null,
        renderedTitle: comparison.renderedText || null,
        impact:
          "Search engines primarily use the raw HTML title; the JS-modified title may not be indexed",
        recommendation:
          "Set the correct title in server-side HTML rather than modifying it with JavaScript",
      });
    }

    return pass("js-title-modified", "Title is unchanged after JavaScript execution", {
