...
export const anchorTextRule = defineRule({
  ...
  description:
    'Checks that link anchor text is descriptive and not generic like "click here" or "read more"',
  category: "links",
  weight: 1,
  run: async (context: AuditContext) => {
    ...
    return pass("links-anchor-text", `All ${links.length} link(s) have descriptive anchor text`, {
      totalLinks: links.length,
    });
  },
});
