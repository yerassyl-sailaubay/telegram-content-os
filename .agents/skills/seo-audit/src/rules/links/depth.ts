...
export const depthRule = defineRule({
  ...
  name: "Page Depth",
  description:
    "Checks that pages are not too deep in the site hierarchy (more than 3 levels from homepage)",
  category: "links",
  weight: 1,
  run: async (context: AuditContext) => {
    ...
    try {
      ...
      return pass("links-depth", "Could not determine page depth from URL", { url, depth: null });
    }

    // Homepage (depth 0) is always good
    if (depth === 0) {
      return pass("links-depth", "This is the homepage (depth 0)", { depth: 0, url });
    }
    ...
    if (depth > MAX_RECOMMENDED_DEPTH) {
      return warn(
        ...
        `Page is ${depth} levels deep (recommended: ${MAX_RECOMMENDED_DEPTH} or less)`,
        {
          ...
          url,
          suggestion:
            "Consider restructuring site hierarchy or adding more internal links to reduce page depth",
        },
      );
    }

    return pass("links-depth", `Page depth is ${depth} level(s) from homepage`, {
      depth,
      maxRecommended: MAX_RECOMMENDED_DEPTH,
      url,
    });
