...
export const httpsDowngradeRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    if (!url.startsWith("https://")) {
      return pass("links-https-downgrade", "Page is not served over HTTPS, check not applicable", {
        pageProtocol: "http",
      });
    }
    ...
    return pass("links-https-downgrade", "All links maintain HTTPS security", {
