 ...
 */
const LOCAL_PATH_PREFIXES = ["/Users/", "/home/", "C:\\", "C:/", "D:\\", "D:/"];
...
export const localFileRule = defineRule({
  ...
  description:
    "Checks for file:// protocol links and absolute OS filesystem paths in href/src attributes",
  category: "links",
  weight: 8,
  run: (context: AuditContext) => {
    ...
    if (found.length > 0) {
      return fail("links-local-file", `Found ${found.length} local file reference(s)`, {
    ...
    return pass("links-local-file", "No local file references found");
  },
});
