...
export const fontSizeRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    // Evaluate results
    if (issues.length === 0) {
      return pass("mobile-font-size", "No mobile font size issues detected", {
        note: "Static analysis - verify on actual mobile device",
      });
    }

    const criticalIssues = issues.filter(
      (i) => i.issue.includes("below 12px") || i.issue.includes("!important"),
    );
    ...
    return warn("mobile-font-size", `Found ${issues.length} mobile font size issue(s)`, {
      issues: issues.slice(0, 10),
      totalIssues: issues.length,
      smallFontCount,
      tightLineHeightCount,
