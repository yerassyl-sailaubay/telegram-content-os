  ...
  run: (context: AuditContext) => {
    ...
    $("form").each((_, el) => {
      ...
      const hasSubscribeButton =
        text.includes("subscribe") || text.includes("sign up") || text.includes("get updates");
    ...
    $('[style*="position: fixed"], [style*="position:fixed"]').each((_, el) => {
      ...
      const isAtTop =
        style.includes("top: 0") || style.includes("top:0") || !style.includes("bottom");
    ...
    // Evaluate results
    if (issues.length === 0) {
      return pass("mobile-interstitials", "No intrusive interstitials detected", {
    ...
    return warn("mobile-interstitials", `Found ${issues.length} potential interstitial issue(s)`, {
      issues: issues.slice(0, 10),
      totalIssues: issues.length,
      recommendation: "Review detected popups to ensure they don't cover main content on mobile",
    });
  },
});
