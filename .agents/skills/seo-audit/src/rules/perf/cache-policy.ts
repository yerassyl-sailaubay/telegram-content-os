...
export const cachePolicyRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    if (hasCacheControl) {
      ...
      if (maxAge > 0) {
        return pass("perf-cache-policy", `Cache policy configured with max-age=${maxAge}s`, {
          ...details,
    ...
    if (hasExpires) {
      return pass("perf-cache-policy", `Cache expiry set via Expires header: ${expires}`, details);
    }
