...
export const domSizeRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    if (stats.totalNodes > THRESHOLDS.nodes.warning) {
      issues.push(
        `${stats.totalNodes.toLocaleString()} DOM nodes (recommended: <${THRESHOLDS.nodes.good})`,
      );
      severity = "fail";
    ...
    if (stats.maxChildren > THRESHOLDS.children.good) {
      issues.push(
        `Max children: ${stats.maxChildren} in ${stats.widestSelector} (recommended: <${THRESHOLDS.children.good})`,
      );
      if (severity === "pass") severity = "warn";
    }
