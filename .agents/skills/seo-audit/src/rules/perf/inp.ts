...
export const inpRule = defineRule({
  ...
  description: "Measures responsiveness by checking the latency of user interactions",
  category: "perf",
  weight: 20,
  run: async (context: AuditContext) => {
    ...
    if (inp <= INP_POOR) {
      return warn("cwv-inp", `INP is ${inp}ms (needs improvement, should be under 200ms)`, {
        metric: "INP",
        value: inp,
        valueFormatted: `${inp}ms`,
        threshold: {
    ...
    return fail("cwv-inp", `INP is ${inp}ms (poor, should be under 200ms)`, {
      metric: "INP",
      value: inp,
      valueFormatted: `${inp}ms`,
      threshold: {
