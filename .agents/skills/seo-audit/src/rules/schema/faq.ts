...
export const structuredDataFaqRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (faqPages.length === 0) {
      return pass("schema-faq", "No FAQPage schema found (not required)", { faqPagesFound: 0 });
    }
    ...
    for (const faqPage of faqPages) {
      ...
      for (let i = 0; i < mainEntity.length; i++) {
        ...
        if (!question.acceptedAnswer) {
          issues.push(`FAQPage Question #${questionIndex}: missing "acceptedAnswer" property`);
          continue;
        }
    ...
    if (issues.length > 0) {
      return fail("schema-faq", `FAQPage schema validation failed: ${issues.join("; ")}`, {
        faqPagesFound: faqPages.length,
        issues,
      });
    }

    return pass("schema-faq", `Valid FAQPage schema found with ${totalQuestions} question(s)`, {
      faqPagesFound: faqPages.length,
