...
export const structuredDataTypeRule = defineRule({
  ...
  description: "Checks that JSON-LD structured data includes the required @type field",
  category: "schema",
  weight: 25,
  run: async (context: AuditContext) => {
    ...
    if (jsonLdScripts.length === 0) {
      return warn("schema-type", "No JSON-LD scripts found to check for @type", { found: false });
    }
    ...
    const recognizedTypes = allTypes.filter((t) => COMMON_SCHEMA_TYPES.includes(t));
    const uniqueTypes = [...new Set(allTypes)];

    if (scriptsWithType === 0 && parseErrors === 0) {
      return fail("schema-type", "No JSON-LD scripts contain a @type field", {
    ...
    if (scriptsWithType === 0 && parseErrors > 0) {
      return fail("schema-type", "Could not check @type - all JSON-LD scripts have parse errors", {
        totalScripts: jsonLdScripts.length,
        scriptsWithType: 0,
        scriptsWithoutType,
        parseErrors,
