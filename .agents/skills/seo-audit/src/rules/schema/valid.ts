...
export const structuredDataValidRule = defineRule({
  ...
  description: "Checks that all JSON-LD scripts contain valid, parseable JSON",
  category: "schema",
  weight: 25,
  run: async (context: AuditContext) => {
    ...
    if (jsonLdScripts.length === 0) {
      return warn("schema-valid", "No JSON-LD scripts found to validate", { found: false });
    }
    ...
    if (invalidCount === 0) {
      return pass("schema-valid", `All ${validCount} JSON-LD script(s) contain valid JSON`, {
    ...
    if (validCount === 0) {
      return fail("schema-valid", `All ${invalidCount} JSON-LD script(s) contain invalid JSON`, {
        totalScripts: jsonLdScripts.length,
        validCount: 0,
        invalidCount,
