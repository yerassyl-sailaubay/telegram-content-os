...
const SECRET_PATTERNS: Array<{ name: string; pattern: RegExp }> = [
  ...
  {
    name: "Slack Webhook",
    pattern: /hooks\.slack\.com\/services\/T[A-Z0-9]{8,}\/B[A-Z0-9]{8,}\/[a-zA-Z0-9]{20,}/g,
  },
  ...
  // Basic Auth: username must be alphanumeric, password must not contain / or quotes, host must have a dot
  {
    ...
    pattern:
      /https?:\/\/[a-zA-Z0-9_-]+:[^/@\s"'<>]+@[a-zA-Z0-9][-a-zA-Z0-9]*(?:\.[a-zA-Z0-9][-a-zA-Z0-9]*)+/gi,
  },
  {
    name: "Bearer Token (JWT)",
    pattern: /bearer\s+eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/gi,
  },
  ...
  // Database connection strings (stricter: username alphanumeric, password no slashes, host must have dot or be localhost)
  {
    name: "MongoDB URI",
    pattern:
      /mongodb(?:\+srv)?:\/\/[a-zA-Z0-9_-]+:[^/@\s"'<>]+@(?:localhost|[a-zA-Z0-9][-a-zA-Z0-9]*(?:\.[a-zA-Z0-9][-a-zA-Z0-9]*)+)/gi,
  },
  {
    name: "PostgreSQL URI",
    pattern:
      /postgres(?:ql)?:\/\/[a-zA-Z0-9_-]+:[^/@\s"'<>]+@(?:localhost|[a-zA-Z0-9][-a-zA-Z0-9]*(?:\.[a-zA-Z0-9][-a-zA-Z0-9]*)+)/gi,
  },
  {
    name: "MySQL URI",
    pattern:
      /mysql:\/\/[a-zA-Z0-9_-]+:[^/@\s"'<>]+@(?:localhost|[a-zA-Z0-9][-a-zA-Z0-9]*(?:\.[a-zA-Z0-9][-a-zA-Z0-9]*)+)/gi,
  },
...
export const leakedSecretsRule = defineRule({
  ...
  run: (context: AuditContext) => {
    ...
    const byType = leakedSecrets.reduce(
      (acc, secret) => {
