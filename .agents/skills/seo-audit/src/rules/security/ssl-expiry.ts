...
export const sslExpiryRule = defineRule({
  ...
  description:
    "Checks SSL health indicators via HSTS max-age as a proxy for certificate maintenance",
  category: "security",
  weight: 8,
  run: (context: AuditContext) => {
    ...
    if (!url.startsWith("https://")) {
      return fail("security-ssl-expiry", "Page is served over HTTP; no SSL certificate in use", {
        isHttps: false,
    ...
    if (!hstsHeader) {
      return warn(
        ...
        "HTTPS is enabled but no Strict-Transport-Security header found; browsers can be downgraded to HTTP",
        {
          ...
          hasHsts: false,
          recommendation:
            "Add Strict-Transport-Security header with max-age of at least 1 year (31536000 seconds)",
        },
      );
    }
    ...
    if (maxAge === null) {
      return warn("security-ssl-expiry", "HSTS header present but max-age is missing or invalid", {
        isHttps: true,
        hasHsts: true,
        hstsHeader,
    ...
    if (maxAge < ONE_DAY) {
      return warn(
        ...
        `HSTS max-age is very short (${maxAge} seconds / ${(maxAge / 3600).toFixed(1)} hours); this provides minimal protection`,
        {
          ...
          recommendation:
            "Increase HSTS max-age to at least 1 year (31536000 seconds) for proper browser enforcement",
        },
      );
    }
