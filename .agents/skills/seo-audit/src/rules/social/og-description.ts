...
export const ogDescriptionRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!content) {
      return fail("social-og-description", "Open Graph description tag exists but has no content", {
        found: true,
        empty: true,
      });
    }

    return pass("social-og-description", "Open Graph description tag is present", {
      found: true,
