...
export const ogImageSizeRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!ogImage) {
      return fail("social-og-image-size", "No og:image found - cannot check dimensions", {
        hasImage: false,
      });
    }
    ...
    if (
      (widthMeta && (width === null || isNaN(width))) ||
      (heightMeta && (height === null || isNaN(height)))
    ) {
      return warn("social-og-image-size", "og:image dimension meta tags contain invalid values", {
        hasImage: true,
        ogImage,
        widthMeta,
        heightMeta,
    ...
    if (width >= RECOMMENDED_WIDTH && height >= RECOMMENDED_HEIGHT) {
      return pass("social-og-image-size", `og:image has optimal dimensions (${width}x${height})`, {
        hasImage: true,
        ogImage,
        width,
        height,
