...
export const ogImageRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (ogImageElement.length === 0) {
      return fail("social-og-image", 'No <meta property="og:image"> tag found in the document', {
        found: false,
      });
    }
    ...
    if (!content) {
      return fail("social-og-image", "Open Graph image tag exists but has no content", {
        found: true,
        empty: true,
      });
    }
    ...
    return pass("social-og-image", "Open Graph image tag is present with a valid URL", {
      found: true,
      ogImage: content,
      isValidUrl: true,
    });
