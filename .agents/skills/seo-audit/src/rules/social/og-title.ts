...
export const ogTitleRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (ogTitleElement.length === 0) {
      return fail("social-og-title", 'No <meta property="og:title"> tag found in the document', {
        found: false,
      });
    }
    ...
    if (!content) {
      return fail("social-og-title", "Open Graph title tag exists but has no content", {
        found: true,
        empty: true,
      });
    }

    return pass("social-og-title", "Open Graph title tag is present", {
      found: true,
      ogTitle: content,
    });
  },
