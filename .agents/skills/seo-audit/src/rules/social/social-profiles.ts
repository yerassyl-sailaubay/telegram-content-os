...
const SOCIAL_PROFILE_PATTERNS = [
  {
    platform: "Facebook",
    pattern: /^https?:\/\/(www\.)?facebook\.com\/(?!sharer|share)[\w.-]+\/?$/i,
  },
  {
    platform: "Twitter/X",
    pattern: /^https?:\/\/(www\.)?(twitter|x)\.com\/(?!intent|share)[\w]+\/?$/i,
  },
...
export const socialProfilesRule = defineRule({
  ...
  description: "Checks for links to social media profiles in header, footer, or structured data",
  category: "social",
  weight: 1,
  run: async (context: AuditContext) => {
    ...
    if (profileCount > 0) {
      return warn(
        ...
        `Only ${profileCount} social profile${profileCount === 1 ? "" : "s"} found (${platformList.join(", ")}). Consider adding more for brand presence`,
        {
          ...
          suggestion:
            "Add links to major platforms like Facebook, Twitter/X, Instagram, or LinkedIn",
        },
      );
    }
    ...
    return warn(
      ...
      "No social media profile links found. Add links to your social profiles in header/footer",
      {
        ...
        profileCount: 0,
        suggestion:
          "Add social profile links in header/footer and include sameAs in Organization schema",
      },
    );
  },
});
