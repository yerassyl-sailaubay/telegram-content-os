 ...
 */
const VALID_CARD_TYPES = ["summary", "summary_large_image", "app", "player"] as const;
...
export const twitterCardRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (!content) {
      return fail("social-twitter-card", "Twitter card tag exists but has no content", {
        found: true,
        empty: true,
      });
    }
    ...
    return pass("social-twitter-card", "Twitter card tag is present with a valid type", {
      found: true,
      twitterCard: content,
      isValidType: true,
    });
