...
export const fourXxNon404Rule = defineRule({
  ...
  description: "Checks for 4xx client errors other than 404 (e.g., 401, 403, 410, 429)",
  category: "technical",
  weight: 8,
  run: async (context: AuditContext) => {
    ...
    if (statusCode === 404) {
      return pass("technical-4xx-non-404", "Page returns 404 (handled by dedicated 404 rules)", {
        statusCode,
      });
    }
    ...
    if (statusCode < 400 || statusCode >= 500) {
      return pass("technical-4xx-non-404", "No client error detected", { statusCode });
    }
    ...
    if (statusCode === 401) {
      return fail("technical-4xx-non-404", `Page requires authentication (401 ${label})`, {
        statusCode,
        label,
    ...
    if (statusCode === 403) {
      return fail("technical-4xx-non-404", `Page is forbidden (403 ${label})`, {
        statusCode,
        label,
    ...
    if (statusCode === 410) {
      return warn("technical-4xx-non-404", `Page is permanently gone (410 ${label})`, {
        statusCode,
        label,
    ...
    if (statusCode === 429) {
      return warn("technical-4xx-non-404", `Rate limited (429 ${label})`, {
        statusCode,
        label,
    ...
    return fail("technical-4xx-non-404", `Page returned client error ${statusCode} ${label}`, {
      statusCode,
      label,
