 ...
 */
const VALID_HTML_CONTENT_TYPES = ["text/html", "application/xhtml+xml"];
...
export const badContentTypeRule = defineRule({
  ...
  description: "Checks that the Content-Type header is correct for an HTML page",
  category: "technical",
  weight: 8,
  run: async (context: AuditContext) => {
    ...
    if (!contentType) {
      return warn("technical-bad-content-type", "Content-Type header is missing", {
        contentType: null,
    ...
    const isValid = VALID_HTML_CONTENT_TYPES.some((valid) => mediaType === valid);

    if (isValid) {
      return pass("technical-bad-content-type", `Content-Type is correct: ${contentType}`, {
        contentType,
        mediaType,
      });
    }
