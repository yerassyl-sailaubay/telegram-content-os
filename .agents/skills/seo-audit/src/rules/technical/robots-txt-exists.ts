...
export const robotsTxtExistsRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    try {
      ...
      if (statusCode === 200) {
        return pass("technical-robots-txt-exists", "robots.txt file exists and is accessible", {
          url: robotsTxtUrl,
          statusCode,
        });
      }
      ...
      return fail("technical-robots-txt-exists", `Failed to check robots.txt: ${message}`, {
        url: robotsTxtUrl,
        error: message,
      });
    }
