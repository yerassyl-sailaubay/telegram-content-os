...
export const serverErrorRule = defineRule({
  ...
  run: async (context: AuditContext) => {
    ...
    if (statusCode >= 500) {
      ...
      return fail("technical-server-error", `Page returned ${statusCode} ${label}`, {
        statusCode,
        label,
    ...
    if (statusCode >= 200 && statusCode < 300) {
      return pass("technical-server-error", "Page returned a successful response", { statusCode });
    }

    // Other status codes (3xx, 4xx) are handled by dedicated rules
    return pass("technical-server-error", "No server error detected", { statusCode });
  },
});
