 ...
 */
const TITLE_SOFT_404_PATTERNS = ["404", "not found", "page not found", "error"];

/**
 * Patterns in the H1 that indicate a soft 404
 */
const H1_SOFT_404_PATTERNS = ["404", "not found", "page not found"];
...
export const soft404Rule = defineRule({
  ...
  description: "Detects pages that return HTTP 200 but display error page content (soft 404)",
  category: "technical",
  weight: 10,
  run: async (context: AuditContext) => {
    ...
    if (statusCode === 404) {
      return pass("technical-soft-404", "Page correctly returns 404 status code", { statusCode });
    }
    ...
    if (wordCount < MIN_WORD_COUNT) {
      ...
      if (bodyMatch) {
        indicators.push(`Body has thin content (${wordCount} words) and contains "${bodyMatch}"`);
      }
    }
    ...
    return pass("technical-soft-404", "Page does not appear to be a soft 404", {
      statusCode,
      wordCount,
    });
  },
