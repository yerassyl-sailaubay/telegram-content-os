...
export const timeoutRule = defineRule({
  ...
  description: "Checks if the page response time is excessive, indicating a near-timeout condition",
  category: "technical",
  weight: 8,
  run: async (context: AuditContext) => {
    ...
    return pass("technical-timeout", `Page response time is ${responseTime}ms`, {
      responseTimeMs: responseTime,
    });
  },
});
