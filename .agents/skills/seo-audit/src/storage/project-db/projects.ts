...
export function getOrCreateProject(
  ...
  const existing = db.prepare("SELECT * FROM projects WHERE domain = ?").get(domain) as
    | DbProject
    | undefined;
 ...
 */
export function getProjectById(db: Database.Database, id: number): HydratedProject | null {
 ...
 * @returns Project record or null
 */
export function getProjectByDomain(db: Database.Database, domain: string): HydratedProject | null {
  const row = db.prepare("SELECT * FROM projects WHERE domain = ?").get(domain) as
    | DbProject
    | undefined;
...
export function listProjects(db: Database.Database): HydratedProject[] {
  const rows = db.prepare("SELECT * FROM projects ORDER BY updated_at DESC").all() as DbProject[];
