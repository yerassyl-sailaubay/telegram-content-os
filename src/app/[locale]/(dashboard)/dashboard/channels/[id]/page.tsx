...
import { ArrowLeft, Users, FileText, Calendar, Unplug } from "lucide-react";
import { redirect } from "next/navigation";
import { PostActions } from "@/components/channels/post-actions";
  ...
  return (
    <>
      <PageHeader
        ...
        description={channel.description ?? (channel.username ? `@${channel.username}` : undefined)}
        actions={
        ...
        <div className="space-y-4 lg:col-span-2">
              ...
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div>
                  <p className="text-sm font-semibold">
                    {channel.lastPostAt ? formatDistanceToNow(new Date(channel.lastPostAt)) : "—"}
                  </p>
                  <p className="text-muted-foreground text-xs">{t("lastPost")}</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Last {recentPosts.length} imported posts</CardDescription>
            </CardHeader>
            <CardContent>
              {recentPosts.length === 0 ? (
                <div className="flex min-h-[120px] items-center justify-center rounded-lg border border-dashed">
                  <p className="text-muted-foreground text-sm">{t("noRecentPosts")}</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {recentPosts.map((post) => (
                    <div key={post.id} className="bg-muted/30 rounded-lg border p-3">
                      <div className="flex items-start justify-between gap-2">
                        <p className="line-clamp-2 flex-1 text-sm">{post.contentRaw || "—"}</p>
                        <PostActions post={post} channel={channel} />
                      </div>
                      <div className="text-muted-foreground mt-2 flex items-center gap-3 text-xs">
                        {post.postedAt && (
                          <span>{formatDistanceToNow(new Date(post.postedAt))}</span>
                        )}
                        {typeof post.views === "number" && post.views > 0 && (
                          <span>{post.views.toLocaleString()} views</span>
                        )}
                        {typeof post.forwards === "number" && post.forwards > 0 && (
