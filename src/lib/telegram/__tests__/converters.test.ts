...
describe("toMarkdown", () => {
  ...
  it("converts bold text", () => {
    const parsed = parsedFromText("Hello bold World", [
      makeEntity("bold", 6, 4),
    ]);
  ...
  it("converts italic text", () => {
    const parsed = parsedFromText("Hello italic World", [
      makeEntity("italic", 6, 6),
    ]);
    expect(toMarkdown(parsed)).toContain("_italic_");
  });

  it("converts underline text", () => {
    const parsed = parsedFromText("Hello underline World", [
      makeEntity("underline", 6, 9),
    ]);
    expect(toMarkdown(parsed)).toContain("<u>underline</u>");
  });

  it("converts strikethrough text", () => {
    const parsed = parsedFromText("Hello strike World", [
      makeEntity("strikethrough", 6, 6),
    ]);
    expect(toMarkdown(parsed)).toContain("~~strike~~");
  });

  it("converts inline code", () => {
    const parsed = parsedFromText("Use code here", [
      makeEntity("code", 4, 4),
    ]);
  ...
  it("converts spoiler", () => {
    const parsed = parsedFromText("The answer is secret", [
      makeEntity("spoiler", 14, 6),
    ]);
  ...
  it("converts blockquote", () => {
    const parsed = parsedFromText("Quoted text", [
      makeEntity("blockquote", 0, 11),
    ]);
  ...
  it("converts URL entity", () => {
    const parsed = parsedFromText("Visit https://example.com today", [
      makeEntity("url", 6, 19),
    ]);
  ...
  it("converts mention", () => {
    const parsed = parsedFromText("Hello @user!", [
      makeEntity("mention", 6, 5),
    ]);
    expect(toMarkdown(parsed)).toContain("@user");
  });

  it("converts hashtag", () => {
    const parsed = parsedFromText("Check #trending", [
      makeEntity("hashtag", 6, 9),
    ]);
    expect(toMarkdown(parsed)).toContain("#trending");
  });

  it("converts bot command", () => {
    const parsed = parsedFromText("/start now", [
      makeEntity("bot_command", 0, 6),
    ]);
    expect(toMarkdown(parsed)).toContain("`/start`");
  });

  it("converts email", () => {
    const parsed = parsedFromText("Email user@test.com", [
      makeEntity("email", 6, 13),
    ]);
    expect(toMarkdown(parsed)).toContain(
  ...
  it("includes media references", () => {
    const msg = makeMessage({
      caption: "Photo",
      photo: [
        { file_id: "abc123", file_unique_id: "u1", width: 800, height: 600 },
      ],
    });
    const parsed = parseTelegramMessage(msg);
    const md = toMarkdown(parsed);
...
describe("toHTML", () => {
  ...
  it("converts spoiler", () => {
    const parsed = parsedFromText("Answer is secret", [
      makeEntity("spoiler", 10, 6),
    ]);
  ...
  it("converts blockquote", () => {
    const parsed = parsedFromText("Quoted", [
      makeEntity("blockquote", 0, 6),
    ]);
    expect(toHTML(parsed)).toBe("<blockquote>Quoted</blockquote>");
  });

  it("converts expandable blockquote", () => {
    const parsed = parsedFromText("Expandable", [
      makeEntity("expandable_blockquote", 0, 10),
    ]);
    expect(toHTML(parsed)).toBe(
      "<details><blockquote>Expandable</blockquote></details>",
    );
  });
  ...
  it("converts text_link", () => {
    ...
    expect(html).toContain(
      '<a href="https://example.com" rel="noopener noreferrer">here</a>',
    );
  });
  ...
  it("converts hashtag", () => {
    const parsed = parsedFromText("Check #tag", [
      makeEntity("hashtag", 6, 4),
    ]);
  ...
  it("converts email", () => {
    ...
    expect(toHTML(parsed)).toContain(
      '<a href="mailto:user@test.com">user@test.com</a>',
    );
  });

  it("converts phone number", () => {
    const parsed = parsedFromText("Call +123456", [
      makeEntity("phone_number", 5, 7),
    ]);
  ...
  it("escapes HTML in code blocks", () => {
    const parsed = parsedFromText('<div class="x">', [
      makeEntity("pre", 0, 15),
    ]);
    const html = toHTML(parsed);
  ...
  it("escapes HTML in mention text", () => {
    const parsed = parsedFromText("@<script>", [
      makeEntity("mention", 0, 9),
    ]);
